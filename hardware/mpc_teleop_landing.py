...
def ref_position(ref):
    return [float(ref[0, 0]), float(ref[0, 2]), float(ref[0, 4])]
...
def main():
    ...
    with SyncCrazyflie(uri, cf=Crazyflie(rw_cache=cache_dir)) as scf:
        ...
        # Wait for OptiTrack
        print(
            f"Waiting for OptiTrack drone + {TRACKED_OBJECT_NAME}...",
            end="",
            flush=True,
        )
        t0 = time.monotonic()
        while (
            reader.drone_count() < MIN_POSE_COUNT
            or reader.target_count() < MIN_POSE_COUNT
        ):
        ...
        print()
        print(f"=== MPC Tracker — following {TRACKED_OBJECT_TOPIC} ===")
        print("Press SPACE to take off, Esc to abort")
        print(f"Tracking target is the OptiTrack rigid body '{TRACKED_OBJECT_NAME}'")
        ...
        with SafeCommander(cf.commander) as commander:
            ...
            try:
                ...
                print(
                    f"MPC tracking active — following {TRACKED_OBJECT_NAME} "
                    f"(log: {log_path.name})"
                )
                step = 0
                hover_target = None
                while not stop.is_set():
                    ...
                    if hover_target is None:
                        hover_target = [drone.pos[0], HOVER_ALTITUDE, drone.pos[2]]

                    if target is None:
                        ref = static_reference(hover_target, N, CONTROL_DT)
                    else:
                        hover_target = [target.pos[0], HOVER_ALTITUDE, target.pos[2]]
                        ref = tracking_reference(
                            rigid_body_to_state(drone),
                            rigid_body_to_state(target),
                            N,
                            CONTROL_DT,
                        )

                    target_pos = ref_position(ref)
                    ...
                    if step % 5 == 0:
                        ...
                        sys.stdout.write(
                            ...
                            f"{TRACKED_OBJECT_NAME}=({target_pos[0]:+.2f},{target_pos[1]:+.2f},{target_pos[2]:+.2f}) "
                        ...
                        pub.publish(MPC_TARGET_TOPIC, json.dumps({"pos": target_pos}))
