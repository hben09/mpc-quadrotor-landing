...
```
mpc_landing/          # Main control package
...
2. **Controller** (PID or MPC) computes desired roll, pitch, yaw, throttle as PWM (1000–2000 µs)
3. **CRSF encoding** (`crsf.py`) packs into 26-byte frames
4. **Serial** (pyserial, 115200 baud, `/dev/ttyACM0`) → ELRS TX module → drone's Betaflight FC
...
- **Python**: numpy, cvxpy, scipy, pyserial
