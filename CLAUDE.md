...
- **`hardware/teleop.py` stays barebones** — manual attitude flight only. Do NOT wire MPC, MQTT publishing, battery logging, or other telemetry into it. It exists as the minimal "just fly the drone" fallback. Put such features in `mpc_pilot.py` instead.
...
| Coordinate mapping | `cf_to_mpc_state()` in `sim/mpc_controller.py` | `optitrack_to_mpc_state()` in `hardware/mpc_pilot.py` |
